...
活动
 -
  ##### 活动目前有三类:
   ...
   3.  有排期的，简单独立的通过ActivityScheduleHelper.getValidSchedule调用实现的
...
配置解析
 - 
  - 要解析配置，生成对象。可以使用xml解析工具
  - 生成两个类，一个是parser, 主要是解析配置数据；另一个是config，主要是生成对象
  - parser 会把对应的ItemSpec生成一个对象，相同的ItemSpec结构会公用一个对象
  - config 默认的变量都是String，需要适当修改，改成对应的类型；int等等
  
  - 具体流程，christmas2019_store.xml 为例
    > 1. xml配置好，提交到svn
    > 2. 浏览器访问http://10.0.3.187:8080/gameservice/generateCode?zoneId=1&xmlFile=christmas2019_store
    > 3. 会自动下载一个压缩包，解压放到桌面。有两个文件 Christmas2019StoreConfig.class 和  Christmas2019StoreParser.class
    > 4. Christmas2019StoreParser.class 放到 com.elex.cok.utils.xml.parser包下，Christmas2019StoreConfig放到com.elex.cok.utils.xml.config 包下
...
new_server文件夹
 - 
 - new_server文件夹只在新服期间生效，当新服转老服时，会重新刷新配置，不会读new_server
 - 这个文件的配置主要作用于，新服结束后，尽管新服排期与全服排期有交叉，也会进入全服排期。适用于排期比较长的活动
 
活动仪表板
 -
  - 目前仪表板仅供参考，仪表板的数据与实际生效的数据是互相独立的，可能存在不一致的情况
...
大地图锁点
 -
 ##### WorldPoint 单点加锁改造完成，原来的 update() 方法废弃，拆成两个调用 skipUpdate() & strictUpdate()

 > 两个方法的业务逻辑是完全一样的： <br/>
 > 更新单点时如果有锁冲突，抛出预警直接跳过，否则更新  <br/>
 > 两个方法的区别是： <br/>
 > strictUpdate() 进行更新时，没有提前获取锁，直接抛出预警（即使没有锁冲突） <br/>
 

##### 所以这两个方法主要是用来区分调用业务场景。调用 strictUpdate() 需要有锁，skipUpdate() 可以直接调用。

  > strictUpdate() 为严格更新，调用此方法前需要保证点 在更新前有锁，来保证不会遇到更新时的点锁冲突 <br/>
    此方法为强业务相关，大部分单点更新应考虑调用此方法。比如刷联盟堡垒，如果点更新失败了后面的所有逻辑都不应该继续执行，否则会逻辑不统一
 

   > skipUpdate() 为跳过更新，调用此方法无需保证点锁，也就是业务上允许冲突时跳过，比如刷野怪，刷资源田（少刷几只没事）

   > 又比如逻辑上可以进行判定：清理4格建筑时，4格建筑的主点需要获取锁用 strictUpdate()，而周边点的清理则可以使用 skipUpdate()（因为主点锁定了，周边点必然是没有其他线程锁定的
